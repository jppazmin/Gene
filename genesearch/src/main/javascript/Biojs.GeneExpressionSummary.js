...
{	
	...
	_drawTemplate: function(){
		...
		this._titleContainer = jQuery('<div class="GeneExpressionSummary_title"></div>').appendTo(this._container);
		
		this._title = jQuery('<span>Loading...</span>').appendTo(this._titleContainer);
		
		this._compactButton = jQuery('<span style="float:right;cursor:pointer;" class="toggle collapse"></span>')
			.appendTo(this._titleContainer)
			.click(function(e){
				self._table.hide();
				self._footerContainer.hide();
				self._titleContainer.children('span.toggle').toggle();
				self.opt.collapsed = true;
			});
		
		this._expandButton = jQuery('<span style="float: right; display:none; cursor:pointer;" class="toggle expand"></span>')
			.appendTo(this._titleContainer)
			.click(function(e){
				self._table.show();
				self._footerContainer.show();
				self._titleContainer.children('span.toggle').toggle();
				self.opt.collapsed = false;
			});
		
		this._table = jQuery('<table class="GeneExpressionSummary_table"></table>').appendTo(this._container);
		...
		if ( this.opt.collapsed ) {
			...
			this._titleContainer.children('span.expand').show();
			this._titleContainer.children('span.collapse').hide();
		}
		
	},
	
	/**
     * Hides the summary except the title 
     * 
     * @example 
     * instance.collapse();
     */
	collapse: function ( ) {
		if ( this._compactButton.is(':visible') ) {
			this._compactButton.click();
		}
	},
	
	/**
     * Expands the view of the summary
     * 
     * @example 
     * instance.expand();
     */
	expand: function ( ) {
		if ( this._expandButton.is(':visible') ) {
	...
	_requestFeaturesXML: function( opt ){
		...
		var httpRequest = {
			...
			error: self._processErrorRequest
		};
	...
	_processErrorRequest: function (textStatus){
		...
		this.raiseEvent( Biojs.GeneExpressionSummary.EVT_ON_REQUEST_ERROR, { message: textStatus } );
	},
	/* 
